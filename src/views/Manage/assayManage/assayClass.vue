  ...
  methods:{
    ...
    deleteClass(index,row) {
      this.$confirm('是否删除该分类？','警告',{
        ...
        let fd = new FormData()
        fd.append('id',row.id)
        servicesApi.deleteAssayClassify(fd).then(res=>{
        // axios.post('/api/assay/deleteClassify',fd).then(res=>{
    ...
    submitForm() {
      ...
      if (this.title === '添加分类'){
        servicesApi.addAssayClassify(this.editForm.name).then(res=>{
        // axios.post('/api/assay/addClassify/' + this.editForm.name).then(res=>{
        ...
        servicesApi.updateAssayClassify(updateData).then(res=>{
        // axios.post('/api/assay/updateClassify',{
        //   id: this.editForm.id,
        //   name: this.editForm.name
        // }).then(res=>{
          console.log(res.data)
