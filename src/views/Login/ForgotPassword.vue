    ...
    <div class="main">
      ...
      <div class="login_box">
        ...
        <div class="login_box-in">
          <el-form :rules="rules" :model="userData" ref="userData">
            <div class="user_box">
              <span class="iconfont">&#xe817;</span>
              <el-form-item label="" prop="phone">
                <el-input v-model="userData.phone" type="text" placeholder="电话号码" @change="Search"></el-input>
              </el-form-item>
            </div>
            <div class="pwd_box">
              <span class="iconfont">&#xe775;</span>
              <el-form-item label="" prop="password">
                <el-input v-model="userData.password" type="password" placeholder="新的密码"></el-input>
              </el-form-item>
            </div>
            <div class="pwd_box">
              <span class="iconfont">&#xe775;</span>
              <el-form-item label="" prop="re_password">
                <el-input v-model="userData.re_password" type="password" placeholder="重复密码"></el-input>
              </el-form-item>
            </div>
          </el-form>
<!--          <div class="userbox">-->
...
<!--          </div>-->
          ...
          <br>
          <button type="primary"  class="register_btn" @click="update" v-prevent-re-click>确认修改</button>
...
export default {
  ...
  methods:{
    ...
    update(){
      ...
      if (!this.userData.phone || !this.userData.password || !this.userData.re_password){
        alert("修改信息不能为空。");
      } else {
        if (this.$md5(this.userData.password) === this.$md5(this.userData.re_password)){
          //判断后台是否有账号信息存在
          ...
          // }).then(resp=>{
            // console.log('have:',resp.data);
            if (resp.data.code === "200"){
              this.userData.password = this.userData.password.toString()
              this.userData.re_password = this.userData.re_password.toString()
              //账号存在，进行修改密码操作
              let upData = {
                phone: this.userData.phone,
                password: this.$md5(this.userData.password)
              }
              servicesApi.updatePassword(upData).then(res=>{
              ...
              // }).then(res=>{
                ...
                if (res.data.code === "200"){
                  this.$message({
                    message: "账号" + this.userData.phone + "修改密码成功",
                    type: 'success'
                  })
                  // alert(res.data.msg);
                  //alert("修改成功，请返回登录。");
